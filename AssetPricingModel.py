...
def draw_R2_series(pname, f_mer, f_smb, f_hml, f_rmw, f_cma, daterange):
    ...
    for f in 'MER'.split(','):
        plt.plot(XXX, YYY[f], label='CAMP', color = 'red')              # CAMP chart
    for f in 'MER+SMB+HML'.split(','):
        plt.plot(XXX, YYY[f], label='3 Factor Model', color='black')    # 3 Factor Model
    for f in 'MER+SMB+HML+RMW'.split(','):
        plt.plot(XXX, YYY[f], label='Charter Model', color='green')     # Charter Model
    for f in 'MER+SMB+HML+RMW+CMA'.split(','):
        plt.plot(XXX, YYY[f], label='5 Factor Model', linestyle='--', color='blue') # 5 Factor Model
